...
su ulyaoth -c "wget https://raw.githubusercontent.com/ulyaoth/repository/master/ulyaoth-haproxy/SPECS/ulyaoth-haproxy1.7.spec"

# Install all required dependencies from spec file.
if type dnf 2>/dev/null
then
  dnf builddep -y ulyaoth-haproxy1.7.spec
elif type yum 2>/dev/null
then
  yum-builddep -y ulyaoth-haproxy1.7.spec
fi

# Download all source files as specifiec in spec file.
su ulyaoth -c "spectool ulyaoth-haproxy1.7.spec -g -R"

# Build the actual rpm and source rpm.
su ulyaoth -c "rpmbuild -ba ulyaoth-haproxy1.7.spec"
