...
su ulyaoth -c "wget https://raw.githubusercontent.com/ulyaoth/repository/master/ulyaoth-varnish/SPECS/ulyaoth-varnish5.1.spec"

# Install all required dependencies from spec file.
if type dnf 2>/dev/null
then
  dnf builddep -y ulyaoth-varnish5.1.spec
elif type yum 2>/dev/null
then
  yum-builddep -y ulyaoth-varnish5.1.spec
fi

# Download all source files as specifiec in spec file.
su ulyaoth -c "spectool ulyaoth-varnish5.1.spec -g -R"

# Build the actual rpm and source rpm.
su ulyaoth -c "rpmbuild -ba ulyaoth-varnish5.1.spec"
