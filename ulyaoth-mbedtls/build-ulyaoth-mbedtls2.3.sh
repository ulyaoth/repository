...
su ulyaoth -c "wget https://raw.githubusercontent.com/ulyaoth/repository/master/ulyaoth-mbedtls/SPECS/ulyaoth-mbedtls2.3.spec"

if [ "$arch" != "x86_64" ]
then
sed -i '/BuildArch: x86_64/c\BuildArch: '"$buildarch"'' ulyaoth-mbedtls2.3.spec
fi

if type dnf 2>/dev/null
then
  dnf builddep -y ulyaoth-mbedtls2.3.spec
elif type yum 2>/dev/null
then
  yum-builddep -y ulyaoth-mbedtls2.3.spec
fi

su ulyaoth -c "spectool ulyaoth-mbedtls2.3.spec -g -R"
su ulyaoth -c "rpmbuild -ba ulyaoth-mbedtls2.3.spec"
