%define package_name ulyaoth-nginx-mainline-passenger5
...
Name: ulyaoth-nginx-mainline-passenger5-selinux
Version: 1.0.0
Release: 1%{?dist}
BuildArch: x86_64
Vendor: Ulyaoth
...
%description
This package opens up SELinux so you can use the package ulyaoth-nginx-mainline-passenger5.

%install
install -p -m 644 -D %{SOURCE0} $RPM_BUILD_ROOT%{_datadir}/selinux/packages/%{package_name}/ulyaoth-nginx-mainline-passenger5.pp
...
%files
%defattr(-,root,root)
%dir %{_datadir}/selinux/packages/ulyaoth-nginx-mainline-passenger5
%{_datadir}/selinux/packages/ulyaoth-nginx-mainline-passenger5/ulyaoth-nginx-mainline-passenger5.pp

%pre

%post
if [ $1 -eq 1 ]; then
semodule -i /usr/share/selinux/packages/ulyaoth-nginx-mainline-passenger5/ulyaoth-nginx-mainline-passenger5.pp 2>/dev/null || :
...
%preun
if [ $1 -eq 0 ]; then
semodule -r ulyaoth-nginx-mainline-passenger5 2>/dev/null || :
/usr/sbin/restorecon -R -v /var/cache/nginx/passenger_temp 2>/dev/null || :
/usr/sbin/restorecon -R -v /etc/nginx/modules/passenger 2>/dev/null || :
/usr/sbin/restorecon -R -v /var/log/passenger 2>/dev/null || :
%if %use_systemd
...
- Initial release for fixing SELinux when using package ulyaoth-nginx-mainline-passenger5.