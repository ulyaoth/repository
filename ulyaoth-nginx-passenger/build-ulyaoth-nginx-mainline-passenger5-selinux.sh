...
if type dnf 2>/dev/null
then
  dnf builddep -y ulyaoth-nginx-mainline-passenger5-selinux.spec
elif type yum 2>/dev/null
then
  yum-builddep -y ulyaoth-nginx-mainline-passenger5-selinux.spec
fi

su ulyaoth -c "rpmbuild -ba ulyaoth-nginx-mainline-passenger5-selinux.spec"
